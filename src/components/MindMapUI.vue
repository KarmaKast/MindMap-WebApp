<template>
  <div class="container" :style="this.containerStyle">
    <div id="content">
      <MindMapCanvas :apiUrl="this.$store.state.apiUrl"></MindMapCanvas>
    </div>
    ...
    <div id="UI" style="position: absolute; top: 0px; left: 0px;">
      ...
      >
        ...
        >
          <img src='../assets/ic_more_vert_18px.svg' style="color: rgb(255, 164, 164);" > 
        </button>
        <div id="menuItems" :style="this.mainItemsStyle">
          <button-one :colors="this.colors" style="order:1;" ></button-one>
          <button-two :colors="this.colors" buttonText="Load Database" @takeAction="this.loadDatabase" style="order:2;" ></button-two>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    testAPI() {
      ...
      this.$axios.get(url_ + "/describe").then(result => {
        this.respo = "testing API <br/>" + result.data;
      });
    },
    checkApiUrl() {
      ...
      // check if the valid url is a nodeAPI
      this.testAPI();
      this.apiBaseUrl = url;
    },
    loadDatabase() {
